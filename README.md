...
This extension has been tested with following Jmeter versions:

1. 4.0
2. 5.0
3. 5.3

The file structure of reports in version 4 and 5 hasn't changed, so it is expected that this extension will work for all 4.* and 5.* versions of jmeter.
...
1. Azure devops task(publishhtmlreport) to publish files to be consumed by the extension. When htmlType is selected as 'Jmeter', js files generated inside LoadReports/content/js after running the load are sent to the extension.
2. When js files are sent to the extension, it reads the content of the js files and does an inline append of the js files in the index.html page(main page of the extension). After all js scripts are appended in the html page, the page containing a jmeter load test report is rendered to the user when they click on the 'Published HTML' tab parallel to the Summary tab. 
